...
public class LobbyData extends BasePackable {
    public LobbyData(int instanceID, final PlayerMap playerMap) {
        super(instanceID);
        ...
        putRecord(new ListRecord<>(new BaseRecord.DeltaFunc<List<StringRecord>>() {
            ...
            public List<StringRecord> get() {
                ...
                // server ship
                out.add(new StringRecord(playerMap.getHostShipID(), -1));

                for (PlayerShipDest playerShip : playerMap.getPlayerShips().values()) {
